...
export default function PrincipalHODPending() {
  ...
  const loadRequests = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API}/principal/hod-pending`, {
  ...
  const approveRequest = async () => {
    ...
    try {
      ...
      alert("HOD request approved successfully!");
  ...
  const rejectRequest = async () => {
    ...
    try {
      ...
      alert("HOD request rejected successfully!");
      setSelectedRequest(null);
      setActionType(null);
      setComments("");
      loadRequests();
  ...
  const exportToCSV = () => {
    const headers = ["HOD Name", "Department", "Email", "Leave Type", "Start Date", "End Date", "Days", "Reason"];
    ...
    a.href = url;
    a.download = `hod_pending_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-800 dark:text-white">HOD Pending Requests</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Review and process leave requests from Department Heads</p>
        </div>
        <button
          onClick={exportToCSV}
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-purple-50 dark:bg-purple-950/30 rounded-2xl p-4 text-center">
          <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{requests.length}</p>
        ...
        <div className="bg-blue-50 dark:bg-blue-950/30 rounded-2xl p-4 text-center">
          <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">
            {requests.filter(r => r.leave_category === "medical").length}
          </p>
        ...
        <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded-2xl p-4 text-center">
          <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
            {requests.filter(r => r.leave_category === "casual").length}
          </p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-700 p-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search by HOD name, department, or email..."
            value={searchTerm}
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-gray-900/50 border-b border-slate-200 dark:border-gray-700">
              <tr>
                <th className="text-left px-6 py-4 text-sm font-semibold text-slate-600 dark:text-slate-400">HOD</th>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-md w-full mx-4 shadow-xl">
            <div className="p-5 border-b border-slate-200 dark:border-gray-700">
              <div className="flex items-center gap-2">
                ...
                {actionType === "view" && <User size={24} className="text-brand-600" />}
                <h3 className="text-lg font-semibold text-slate-800 dark:text-white">
                  {actionType === "approve" ? "Approve HOD Request" : 
                   actionType === "reject" ? "Reject HOD Request" : 
                   "Request Details"}
                </h3>
              </div>
            </div>
            ...
            <div className="p-5 space-y-4">
              <div className="space-y-2 text-sm">
                <p><span className="font-medium">HOD:</span> {selectedRequest.full_name}</p>
                ...
                <p><span className="font-medium">Leave Type:</span> {selectedRequest.leave_category}</p>
